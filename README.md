...
> 带登录鉴权的多房间实时聊天室。Express + Socket.IO + SQLite，原生 HTML/CSS/JS 前端，零构建步骤。

## 项目状态：功能完成，测试 228 项全过 ✅
...
| 4 | 注册登录 + JWT 鉴权 | ✅ |
| 5 | 前端登录注册页 | ✅ |
| 6~8 | Socket.IO 实时聊天 / 历史消息 / 在线用户 / 多房间 / 私聊 | ✅ |
| 9 | 测试与优化（含移动端适配、XSS 防护） | ✅ |
| 10 | 部署上线（配置加固 + 部署验证） | ✅ 代码就绪，待推 GitHub |

**已完成的功能**：注册、登录、JWT 鉴权、多房间公共聊天、实时消息推送、历史消息、在线用户列表、一对一私聊、移动端自适应。

**测试**：228 项全过 —— 后端 71（鉴权 18 + Socket 53）、浏览器端 119（登录注册页 52 + 聊天 67）、部署验证 38。

**相关文档**：
- [`design.md`](./design.md) —— 完整技术设计（表结构、API、Socket 事件）
- [`部署指南.md`](./部署指南.md) —— 部署上线完整步骤
- [`第6阶段完成说明.md`](./第6阶段完成说明.md) —— 实时聊天实现细节
- [`第10阶段完成说明.md`](./第10阶段完成说明.md) —— 部署加固与验证
- [`界面改造指南.md`](./界面改造指南.md) —— 界面调整说明
...
```bash
npm test              # 后端全量测试（71 项）
npm run test:deploy   # 部署验证（用线上参数跑，38 项）
npm run test:browser  # 浏览器端到端（需服务已启动，119 项）
npm run test:all      # 后端 + 部署验证
npm run clean:test    # 清理测试产生的账号和消息
```
...
- [x] 能注册
- [x] 能登录
- [x] 密码加密（bcrypt，实测库里无明文）
- [x] 未登录不能聊天（浏览器实测跳转登录页）
- [x] 实时收发消息（双浏览器实例实测）
- [x] 刷新后消息还在
- [x] 在线用户实时变化（多标签页去重）
- [x] 多房间
- [x] 私聊
- [x] 手机能用（390×844 视口实测无横向溢出）
- [x] 部署准备完成（配置加固 + 38 项线上模式验证通过）
- [ ] 部署到公网 ← **需要你的 GitHub 账号，见 [`部署指南.md`](./部署指南.md)**
- [x] README 完整
